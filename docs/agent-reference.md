...
This is the detailed working reference moved from `CLAUDE.md` on 2026-09-14.
Read the section relevant to the layer being changed; do not load this entire
file for routine work. Paths below are relative to the repository root. Phase
introductions record their historical scope. Current entry points and contracts
are in `AGENTS.md`, `CLAUDE.md` and the feature documents they link to.
...
- **`job.remote_apply`, `job.remote_artifact`, `job.remote_log` and `job.remote_run` are still forbidden**, still scanned for in `tests/test_orch_rpc.c`. A worker's `logs/stdout.log` is reachable through `job.remote_result` under no name; **A14R-F's `job.remote_failure` returns a bounded, redacted tail of it** (`ATLAS_ORCH_LOG_TAIL_MAX`, carried by the dispatcher as an ordinary `log_tail` event under its own lease, never named by a caller) beside the ledger and the dispatcher's own `failure` record, and states every absence with its reason — the transcript itself is still reachable from no remote route. The method writes no row, starts no process and applies nothing. **A17 is the stated reversal of this refusal, narrower on four counts** — the applied bytes are Atlas' own stored patch, not a request field; the tree, owner and commands live in a root-owned conf the request never touches; the proposing and confirming credentials must differ; and the root process is an operator-installed agent, never Atlas itself — see `docs/remote-deploy.md`.
...
- **A tool** is one entry in `TOOLS[]` in `src/mcp/mcp_tools.c`: a schema function
  ...
  declare every argument. The shared dispatcher checks argument keys against
  that schema before invoking a handler; typed readers still validate values.
  `tests/test_mcp_arguments.c` exercises every tool on both transports. Add the name to the
