 ...
 *
 * Not a public header, on `src/db/db_internal.h`'s own precedent: `op_to_params`
 * has external linkage only so `tests/test_decision_remote.c` can drive it
 * directly and prove that a REMOTE op is refused before anything is
 * serialised for the socket. No production caller outside
 * `src/core/service_decision.c` includes this.
 */
