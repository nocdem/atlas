/* Atlas - the A16 T5 remote-disposal acceptance daemon.
 * Copyright 2026 The Atlas Authors. Licensed under the Apache License 2.0.
 *
 * Runs the real daemon with a gateway policy injected from a plain file, and
 * exists for one reason: `tests/test_gw_dispose.c` has to reach a daemon that
 * offers `decision.remote_challenge` and `decision.remote_dispose` under a
 * chosen `tls_mode`, a chosen disposal credential and a chosen
 * `operator_accepts_cleartext_disposal` -- and `atlas_gwpolicy_load`'s
 * root-ownership walk can only ever succeed for a genuinely root-owned
 * `/etc/atlas/gateway.conf`. A fixture cannot produce one and must not try
 * to: this reads the file itself, by an ordinary `open`/`read`, deliberately
 * bypassing that walk, and hands the raw bytes to `atlas_daemon_run` through
 * `atlas_daemon_opts.gwpolicy_text`, which is the only production route to
 * an injected policy and is set by no CLI flag -- P0's `watch_budget_total`
 * precedent (`tests/tools/atlas_watch_daemon.c`), for the identical reason: a
 * way to hand a daemon an arbitrary policy is reachable by anyone who can
 * start one, which is not the same set of people who can write
 * `/etc/atlas/gateway.conf`.
 *
 ...
 * thing that differs is where the gateway policy's bytes came from.
 *
 ...
 * a policy that does not come back ENABLED is reported and refused --
 ...
 * offer.
 *
 * Usage: atlas-gw-daemon DATA_DIR POLICY_FILE
 */
#include <errno.h>
#include <fcntl.h>
#include <stdio.h>
...
int main(int argc, char **argv) {
    if (argc != 3) {
        (void)fprintf(stderr, "usage: atlas-gw-daemon DATA_DIR POLICY_FILE\n");
        return 2;
    }
